...
Terrain.optimizeCollisionDataByCell = function(terrainData) {
	//We add one cell on each side for external borders
	const width = terrainData.width()+2
	const height = terrainData.height()+2
	...
	for (let x = 0; x <= width; x++) {
		for (let y = 0; y <= height; y++) {
			if (!terrainData.isWall(x, y)) continue
			...
			//We need some overlapping between rectangles
			if (!coveredByRectangle[x][y]
					|| !coveredByRectangle[x+1][y]
					|| !coveredByRectangle[x][y+1]) {
			}
		}
	}
